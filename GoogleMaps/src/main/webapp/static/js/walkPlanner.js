...
function updateRoadRoute(originPoint, destinationPoint) {
    var walkDistance = 0;
    var time = 0;
    var pathPoints = 0;
    var directionService = new google.maps.DirectionsService;
    var directionServiceOption = {
        origin: originPoint,
        destination: destinationPoint,
        provideRouteAlternatives: false,
        travelMode: google.maps.DirectionsTravelMode.WALKING
    };
    var geoPolyline = new google.maps.Polyline({
        path: [],
        strokeColor: "#6A5ACD",
        strokeOpacity: .8,
        strokeWeight: 5
    });
    directionService.route(directionServiceOption, function(response, status) {
        if (status == google.maps.DirectionsStatus.OK) {
            var f = response.routes[0].overview_path;
            ...
            for (i = 0; i < legs.length; i++) {
                ...
                for (j = 0; j < l.length; j++) {
                    ...
                    for (k = 0; k < m.length; k++) {
                    	polylineOptions.getPath().push(m[k])
                    }
                }
            }
            for (i = 0; i < legs.length; i++) {
            	walkDistance += legs[i].distance.value;
            	time += legs[i].duration.value
            }
            walkDistance = walkDistance / 1e3;
            time = time / 60;
            routePointArray[routePointArray.length - 1].setWalkDistance(walkDistance);
            routePointArray[routePointArray.length - 1].setWalkTime(time);
            routePointArray[routePointArray.length - 1].setPolyline(geoPolyline);
            routePointArray[routePointArray.length - 1].setPathPoints(pathPoints);
